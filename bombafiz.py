...
ITEMS_PER_PAGE = 10  # Стран на странице
...
def get_country_keyboard_page(acc_type, page=0):
    """Клавиатура стран с пагинацией: 10 стран, навигация, кнопка в меню"""
    ...
    # Кнопка в меню всегда отдельно внизу
    keyboard.add(InlineKeyboardButton("[ ← В меню ]", callback_data="main_menu"))
    return keyboard
...
async def country_select(call: CallbackQuery):
    ...
    year_text = f" {account.get('year', '')} года" if acc_type == "отлега" and account.get("year") else ""
    ...
    if acc_type == "обычный":
        keyboard.add(InlineKeyboardButton("[ ← Назад ]", callback_data="buy_regular"))
    else:
